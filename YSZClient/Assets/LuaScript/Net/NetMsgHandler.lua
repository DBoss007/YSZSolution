...
function NetMsgHandler.OnCutOut(isCutOut)
	...
	if isCutOut == true then --切出
		...
		loginNetClient:ShutDown()--关闭raknet
		...
		GameData.OpenInstallRoomID = nil
		GameData.OpenInstallReferralsID = nil
		CutOutTime = os.time() - CutOutTime --计算出切出的总时长
...
function NetMsgHandler.OnClearMessageOK(param)
	...
	CS.LoadingDataUI.Show(5);
	NetMsgHandler.SendCutOutReturn()
end
...
function NetMsgHandler.OnConnectionLost(param)
	...
	if param == 21 then--服务器关闭
		--尝试自动重连
		AutoConnectCount = AutoConnectCount + 1
	elseif param == 32 then--服务器强踢客户端
		--切回登陆界面
		CS.BubblePrompt.Show("连接超时！", "NB")
		isReturnLoginUI = true
	elseif param == 17 then--连接请求无法完成时收到此消息
		--切回登陆界面
		CS.BubblePrompt.Show("连接超时！", "NB")
		isReturnLoginUI = true
	elseif param == 22 then--数据包不能发送到指定系统，与该系统的连接已经关闭
		--尝试自动重连
		AutoConnectCount = AutoConnectCount + 1
	end
...
function NetMsgHandler.ConnectAndSendLogin()
	...
	local realConnectIP
	if string.find(GameConfig.GameServerIP, ":") ~= nil then--当前网络环境为ipv6
...
function NetMsgHandler.HandleReceivedLogin(message)
	...
	if resultType == 0 then
		...
		local roomID = message:PopUInt32() --房间ID; 若ID为0表示在大厅
		-- 设置未读邮件数
		...
		GameData.RoleInfo.ModifyNameCount = message:PopByte()		-- 改名次数
		...
		GameData.IsShowInviteBtn = message:PopByte() 	-- 是否显示邀请码按钮
		GameData.ChannelCode = message:PopByte() 	 	-- 注册初始渠道反馈
...
function NetMsgHandler.SendBindAccount(openid, name)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
...
function NetMsgHandler.Received_CS_Enter_Room(message)
	CS.LoadingDataUI.Hide()
	local resultType = message:PopByte()
	if resultType == 0 then -- 成功需要在发送消息结束后关闭数据加载框
		
		if IsAutoConnect then --断线自动登录流程
			--print("悄悄进房间")
...
function NetMsgHandler.ExitRoomToHall(handleType)
	if GameData.GameState == GAME_STATE.HALL then --如果当前已经是房间状态，直接返回
		return
	end
...
function NetMsgHandler.Send_CS_Request_Statistics(...)
	...
	if length > 0 then
		local message = CS.Net.PushMessage()
		message:PushUInt16(length) -- 写入长度
		for i = 1, length do
...
function NetMsgHandler.ParseAndSetBankerInfo(message)
	...
	GameData.RoomInfo.CurrentRoom.BankerInfo.LeftCount = message:PopByte()		-- 庄家剩余局数
	GameData.RoomInfo.CurrentRoom.BankerInfo.HeadIcon = message:PopByte()			-- 头像ID
	GameData.RoomInfo.CurrentRoom.BankerInfo.IsLastForceDownBanker = message:PopByte() == 1  -- 上一个庄家是否被强制下庄
end
...
function NetMsgHandler.ParseAndSetPokerCards(message, count)
	for index = 1, count, 1 do
		...
		GameData.RoomInfo.CurrentRoom.Pokers[index].Visible = (index % 3 == 1) -- 第一张可见其它不可见
	end
end
...
function NetMsgHandler.ParseAndSetBetRankFirst(message)
	...
	local longIcon = message:PopByte() 	-- 龙头像
	...
	local huIcon = message:PopByte() 	-- 虎头像
...
function NetMsgHandler.SendSmallHorn(contentString)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushString(contentString)--小喇叭字符串信息压入消息结构体
...
function NetMsgHandler.SendEmail(title, content, receiverID, gold)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushByte(2)--邮件发送者类型
	message:PushString(title)--邮件标题
	message:PushString(content)--邮件内容
	message:PushUInt32(receiverID)--接收者id
	message:PushUInt64(gold)--邮件附带金币数
	message:PushUInt32(0)--邮件附带房卡数
...
function NetMsgHandler.SendCheckAccountID(accountID)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushUInt32(accountID)--待检测账号
...
function NetMsgHandler.SendOtherPlayerInfoRequest(accountID)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushUInt32(accountID)--对方账号
...
function NetMsgHandler.SendReadEmail(emailID)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushUInt32(emailID)--邮件id
...
function NetMsgHandler.SendRequestEmails(mailID)
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)	--玩家账号
	message:PushUInt32(mailID)						--客户端最大的邮件id
...
function NetMsgHandler.HandleReceiveEmail(message)
	...
	if EmailMgr:GetMailList() == nil then	--如果邮件管理器的邮件table为nil，初始化为{}
		EmailMgr:InitList()
	end
...
function NetMsgHandler.SendModifyName(newName)
	...
	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushString(newName)--新昵称
...
-----------------C_RETURN_GAME  511----------------------------------------
-- 发送切回确认 此消息已作废
function NetMsgHandler.SendCutOutReturn()
	print('发送切回确认 此消息已作废')
	--[[	local message =CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID);
	NetMsgHandler.SendMessageToGame(ProtrocolID.CS_RECOVER_GAME, message, false);--]]
end
...
function NetMsgHandler.HandleCutOutReturn(message)
	...
	if resultType == 0 then--成功
		--请求基础信息
		NetMsgHandler.SendRequestAccountInfo()
		--print('服务器通知切回确认')
	elseif resultType == 1 then--账号不存在
		CS.BubblePrompt.Show("账号不存在","NB")
		--退回登陆状态
		--NetMsgHandler.CloseConnect()
		NetMsgHandler.ReturnLogin()
	end--]]
...
function NetMsgHandler.SendRequestAccountInfo()
	...
	--[[	local message = CS.Net.PushMessage()
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	NetMsgHandler.SendMessageToGame(ProtrocolID.CS_REQUEST_ACCOUNT_DATA, message, true)--]]
end
...
function NetMsgHandler.HandleAccountInfo(message)
	...
	if resultType == 0 then
		...
		GameData.RoleInfo.AccountName = message:PopString();
		GameData.RoleInfo.DiamondCount = message:PopInt64();
		GameData.RoleInfo.GoldCount = message:PopInt64()
		GameData.RoleInfo.RoomCardCount = message:PopUInt32();
		GameData.RoleInfo.FreePlayTimes = message:PopByte();
		GameData.RoleInfo.VipLevel = message:PopByte();
		GameData.RoleInfo.ChargeCount = message:PopUInt32();
		GameData.RoleInfo.YesterdayRank = message:PopByte()
		--GameData.RoleInfo.YesterdayWinRank = message:PopByte()
		GameData.RoleInfo.YesterdayGoldNum = message:PopInt64()
		--GameData.RoleInfo.YesterdayWinNum = message:PopInt64()
		local roomID = message:PopUInt32();--房间ID; 若ID为0表示在大厅，服务器保证说，如果是自动重连上的，则此处绝对会发0
		
		-- 设置未读邮件数
		GameData.ResetUnreadMailCount(message:PopUInt16())
		GameData.RoleInfo.PromoterStep = message:PopByte()
		GameData.RoleInfo.IsBindAccount = (message:PopByte() == 1)
		GameData.RoleInfo.InviteCode = message:PopUInt32()
		GameData.RoleInfo.ModifyNameCount = message:PopByte()		-- 改名次数
		...
		GameData.RoomInfo.CurrentRoom.RoomID = roomID
		print("服务器发来的玩家基本信息，房间号为 "..roomID)
		if roomID ~= 0 then
			
			if CutOutTime <= 60 then --如果切出时间在规定范围之内
				--请求房间数据
				NetMsgHandler.SendRequestRoomInfo(roomID)
			else
				if GameData.GameState == GAME_STATE.ROOM then  --如果当前游戏状态为房间，走正常退出房间流程
					NetMsgHandler.Send_CS_Exit_Room(1)
				elseif GameData.GameState == GAME_STATE.HALL then
					NetMsgHandler.Send_CS_Exit_Room(1)
					-- 抛出界面刷新事件
					CS.EventDispatcher.Instance:TriggerEvent(EventDefine.UpdateCurrentBigUIInfo, nil)
				else
					CS.BubblePrompt.Show("游戏状态异常，返回登陆","NB")
					--退回登陆状态
					--NetMsgHandler.CloseConnect()
					NetMsgHandler.ReturnLogin()
				end
			...
			if GameData.GameState == GAME_STATE.ROOM then  --如果当前游戏状态为房间，切换界面为大厅
				...
				-- 切换状态为大厅
				GameData.GameState = GAME_STATE.HALL
			elseif GameData.GameState == GAME_STATE.HALL then --如果当前游戏状态为大厅，刷新大厅的玩家数据
				-- 抛出界面刷新事件
				CS.EventDispatcher.Instance:TriggerEvent(EventDefine.UpdateCurrentBigUIInfo, nil)
			else--报异常
				CS.BubblePrompt.Show("游戏状态异常，返回登陆", "NB")
		...
		CS.BubblePrompt.Show(string.format('服务器发来的玩家基本信息 type = [%d]',resultType), "NB")
		--退回登陆状态
		--NetMsgHandler.CloseConnect()
		NetMsgHandler.ReturnLogin()
	end--]]
...
function NetMsgHandler.SendRequestRoomInfo(roomID)
	...
	message:PushUInt32(GameData.RoleInfo.AccountID)--玩家账号
	message:PushUInt32(roomID)
	NetMsgHandler.SendMessageToGame(ProtrocolID.CS_REQUEST_ROOM_DATA, message, true)--]]
end
...
function NetMsgHandler.HandleRoomInfo(message)
	...
	if resultType == 0 then	--切换界面为房间
		...
		print(string.format('返回请求房间信息结果 type = [%d]，返回登陆',resultType))
		CS.BubblePrompt.Show(string.format('返回请求房间信息结果 type = [%d]，返回登陆',resultType), "NB")
		--退回登陆状态
		--NetMsgHandler.CloseConnect()
		NetMsgHandler.ReturnLogin()
	end--]]
